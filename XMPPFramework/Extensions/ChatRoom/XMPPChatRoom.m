...
@interface XMPPChatRoom ()
{
     NSString * currentRequestBareJidStr;
}

@property (nonatomic, strong) NSString * currentRequestBareJidStr;
...
- (NSString *)currentRequestBareJidStr
{
     ...
     dispatch_block_t block = ^{
          result = currentRequestBareJidStr;
     };
...
- (void)setCurrentRequestBareJidStr:(NSString *)_currentRequestBareJidStr
{
     dispatch_block_t block = ^{
          
          currentRequestBareJidStr = _currentRequestBareJidStr;
     };
                                              ...
                                              requestKey:(NSString *)requestKey
{
     ...
     /*
      ...
      <iq type="get" id="aad5ba">
      <query xmlns="aft:groupchat" query_type="get_members" groupid="1" sinceid="1" count="50"></query>
      </iq>
      */
     ...
     [query addAttributeWithName:@"groupid" stringValue:bareChatRoomJidStr];
     [query addAttributeWithName:@"count" stringValue:[NSString stringWithFormat:@"%ld", (unsigned long) group_user_list_count]];
     if (sinceId.length > 0) [query addAttributeWithName:@"sinceid" stringValue:sinceId];
     
     XMPPIQ *iq = [XMPPIQ iqWithType:@"get" elementID:requestKey ? :[xmppStream generateUUID]];
     [iq addChild:query];
                                                ...
                                                completionBlock:(CompletionBlock)completionBlock
{
     ...
     if ([self canSendRequest]) {// we should make sure whether we can send a request to the server
          ...
          attributeMaps[@"groupid"] = bareChatRoomJidStr;
          attributeMaps[@"count"] = [NSString stringWithFormat:@"%ld", (unsigned long)group_user_list_count];
...
- (void)fetchChatRoomListFromServerWithSinceId:(NSString *)sinceId
{
     ...
     dispatch_block_t block = ^{ @autoreleasepool {
          ...
          [self _fetchChatRoomListFromServerWithSinceId:sinceId];
     }};
...
- (void)_fetchChatRoomListFromServerWithSinceId:(NSString *)sinceId
{
     // This is a public method, so it may be invoked on any thread/queue.
     if (!dispatch_get_specific(moduleQueueTag)) return;
     
     // We've already requested the roster from the server.
     if ([self _requestedChatRoom]) return;
     
     /*//we send the request xml as below:
      <iq type="get" id="aad5ba">
      <query xmlns="aft:groupchat" query_type="get_groups" sinceid="1" count="50"></query>
      </iq>
      */
     
     NSXMLElement *query = [NSXMLElement elementWithName:@"query" xmlns:@"aft:groupchat"];
     [query addAttributeWithName:@"query_type" stringValue:@"get_groups"];
     [query addAttributeWithName:@"count" stringValue:[NSString stringWithFormat:@"%ld", (unsigned long)group_list_count]];
     if (sinceId.length > 0) [query addAttributeWithName:@"sinceid" stringValue:sinceId];
...
- (void)transFormDataAndFetchUseListWithArray:(NSArray *)array
{
     ...
     [array enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
          ...
          if ([self autoFetchChatRoomUserList]) {
               [self fetchUserListFromServerWithBareChatRoomJidStr:[[dic objectForKey:@"groupid"] copy] sinceId:nil];
          }
          ...
          @autoreleasepool{
               ...
               if (requestFromServer) {//If we want to request a info from the server when the chat room user list is not exist in local
                    if (!userListArray || [userListArray count] <= 0) {//If there is no data in the laocal,we should request from the server
                         [self fetchUserListFromServerWithBareChatRoomJidStr:bareChatRoomJidStr sinceId:nil];
                    }
               }
          }
     };
...
- (void)handleFetchChatRoomUserListQueryIQ:(XMPPIQ *)iq withInfo:(XMPPBasicTrackingInfo *)basicTrackingInfo
{
     /*
      ...
      <query xmlns="aft:groupchat" query_type="get_members" groupid="1" sinceid="1" count="50">
      [{id:"1","userjid":"13411111111@localhost","nickname":"default"}]</query>
      </iq>
      */
     dispatch_block_t block = ^{ @autoreleasepool {
          ...
          if ([[iq type] isEqualToString:@"result"]) {
               ...
               if ([[query attributeStringValueForName:@"query_type"] isEqualToString:@"get_members"]) {
                    ...
                    // 继续请求剩下的部分
                         ...
                         [self downloadUserListFromServerWithBareChatRoomJidStr:[query attributeStringValueForName:@"groupid"]
                                                                        sinceId:[[userDics lastObject] objectForKey:@"id"]
                                                                     requestKey:[iq elementID]];
                    }
...
- (void)handleFetchChatRoomListQueryIQ:(XMPPIQ *)iq withInfo:(XMPPBasicTrackingInfo *)basicTrackingInfo
{
     ...
     /*
      <iq from="13412345678@localhost" type="result" to="13412345678@localhost/caoyue-PC" id="aad5ba">
      <query xmlns="aft:groupchat" query_type="get_groups" sinceid="1">
...
- (void)xmppStreamDidAuthenticate:(XMPPStream *)sender
{
     ...
     {
          [self fetchChatRoomListFromServerWithSinceId:nil];
     }
}
